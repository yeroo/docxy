...
- [x] update DOCX support and control/MCP documentation with the protection
      matrix, advisory distinction, watermark display, and deliberate fail-closed
      modes
- [x] record final test counts, fixture/evidence paths, and follow-on dependencies
...
- End-to-end evidence: `docxy/src/main.rs` tests
  ...
  `tui_save_preserves_protection_and_watermark_fixture_metadata`; the control
  policy/error and no-mutation evidence is
  `docxy/src/control.rs::tests::package_fixtures_drive_control_policy_errors_and_denial_invariants`.
- Reproducible sandbox capture:
  `target/test-artifacts/docxy/watermark-page-view.txt` (generated by the
  page-view fixture test; intentionally remains a test artifact rather than a
  committed golden image).
- Tracked-changes-only protection remains fail-closed until ordinary edits can
  be emitted automatically as conforming revisions. The follow-on
  `docs/plans/20260829-docx-tracked-change-actions.md` adds revision modeling and
  accept/reject workflows but explicitly excludes automatic tracking, so
  tracked-edit authoring remains a subsequent dependency.
- Forms-only protection remains fail-closed until docxcore models editable form
  fields/content controls and docxy can authorize only those scoped edits. No
  current plan supplies form filling: `docs/plans/20260829-docx-row-content-controls.md`
  explicitly excludes a form-filling UI, so a dedicated follow-on is required.
