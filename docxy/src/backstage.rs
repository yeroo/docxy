...
//! / Print / Export / Close) shown instead of a ribbon when the File tab is
//! chosen. The Open item drives a small folder browser that lists subfolders and
//! `.docx` files; the app renders a live preview of the highlighted document.
//!
...
impl Backstage {
    ...
    /// Re-read the current directory: subfolders + `.docx` files, folders first.
    ...
    /// The full path of the highlighted `.docx`, for preview/opening.
    pub fn selected_docx(&self) -> Option<PathBuf> {
        let e = self.selected()?;
