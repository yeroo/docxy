...
//! / Export / Exit) shown instead of the grid when the File tab is chosen. The
//! Open item drives a small folder browser that lists subfolders and `.xlsx` /
//! `.csv` files; the app renders a live preview of the highlighted workbook.
//!
//! This module holds the pure state and navigation; rendering the preview and
...
pub enum Pane {
    ...
    Browser,
    /// The read-only workbook preview (scrollable).
    Preview,
...
pub struct Backstage {
    ...
    pub sel: usize,
    /// Rendered preview lines for the highlighted workbook (filled by the app).
...
/// Whether `name` is a workbook the browser can open.
pub fn is_openable(name: &str) -> bool {
    let n = name.to_ascii_lowercase();
    n.ends_with(".xlsx") || n.ends_with(".csv") || n.ends_with(".tsv")
}
...
impl Backstage {
    ...
    /// The full path of the highlighted workbook, for preview/opening.
    ...
    /// Activate the highlighted row. Returns `Some(path)` to open a workbook;
...
mod tests {
    ...
    #[test]
    fn lists_workbooks_and_folders_only_folders_first() {
        let tmp = std::env::temp_dir().join("xlsxy_bs_test");
        ...
        assert!(names.contains(&"a.xlsx"));
        assert!(names.contains(&"data.csv"));
        assert!(!names.contains(&"b.txt"));
        let di = names.iter().position(|n| *n == "sub").unwrap();
        let fi = names.iter().position(|n| *n == "a.xlsx").unwrap();
        assert!(di < fi);
        let lock = bs.entries.iter().find(|e| e.name == "~$a.xlsx").unwrap();
        assert!(lock.locked);
        let _ = std::fs::remove_dir_all(&tmp);
    }
