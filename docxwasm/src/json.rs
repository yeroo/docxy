...
// ---------------------------------------------------------------------------
//
// `docx_ctl` (see `bridge::Session::ctl`) takes the same JSON the host<->agent
// wire carries (`{"verb":…,"args":{…}}`), so unlike the rest of this module
// (which only *emits*), this side must also *parse*. `docxwasm` cannot depend
...
// using `push_str`/`quote` above for output, exactly as before.
...
mod tests {
    ...
    fn parses_a_ctl_request_shape() {
        let v = Json::parse(
            r#"{"verb":"doc.replace-range","args":{"start":1,"end":2,"text":"hi","ok":true}}"#,
        )
        .unwrap();
        assert_eq!(v.get_str("verb"), Some("doc.replace-range"));
        let args = v.get("args").unwrap();
        assert_eq!(args.get_usize("start"), Some(1));
        assert_eq!(args.get_usize("end"), Some(2));
